...
export class FFmpeg {
  ...
  off(event, callback) {
    if (event === "log") {
      this.#logEventCallbacks = this.#logEventCallbacks.filter((entry) => entry !== callback);
    } else if (event === "progress") {
      this.#progressEventCallbacks = this.#progressEventCallbacks.filter((entry) => entry !== callback);
    }
  }
  ...
  mount = (fsType, options, mountPoint) => this.#send({
    type: FFMessageType.MOUNT,
    data: { fsType, options, mountPoint }
  });

  unmount = (mountPoint) => this.#send({
